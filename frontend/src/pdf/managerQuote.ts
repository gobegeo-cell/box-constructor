// src/pdf/managerQuote.ts
import jsPDF from "jspdf";
import { estimateCost, fmt } from "../lib/costing";
...
type Args = {
  ...
  // опционально (для таблиц/цветов):
  sideColors?: string[];   // [ ... , lid=4, bottom=5 ]
  logos?: any;             // объект логотипов как в конструкторе
};

export default async function generateManagerQuote(args: Args) {
  ...
  if (PDF_SECTIONS.qr) {
    try {
      ...
      doc.addImage(qrData, "PNG", GRID.headRight.x + (PDF_LAYOUT.logoW ?? 100) - 30 - 30, GRID.headRight.y + (PDF_LAYOUT.logoH ?? 32) + 2, 18, 18);
    } catch {}
  }

  // линия (оставляю твои расширенные поля)
  doc.setDrawColor(...brandRGB); doc.setLineWidth(0.5);
  doc.line(M - 30, GRID.barY, PAGE.W - M + 30, GRID.barY);

  /* ====== PREVIEW (Canvas) — БЕЗ ИСКАЖЕНИЙ ====== */
  if (PDF_SECTIONS.previewFromCanvas) {
    // (опционально) если добавишь в сцену флаг __screenshotMode — можно включать здесь на время снимка
    ...
    if (snap) {
      ...
      // тонкая рамка — по желанию
      doc.setDrawColor(220);
      doc.rect(frameX, frameY, frameW, frameH);
    } else {
      // fallback — как было
      const c1 = (window as any).__boxCanvas as HTMLCanvasElement | undefined;
      const c2 = document.querySelector("#box-canvas canvas") as HTMLCanvasElement | null;
      const c3 = document.querySelector("canvas") as HTMLCanvasElement | null;
      const canvasEl = c1 || c2 || c3 || undefined;
      if (canvasEl && typeof (canvasEl as any).toDataURL === "function") {
        const img = canvasEl.toDataURL("image/png");
        doc.addImage(img, "PNG", GRID.preview.x, GRID.preview.y, GRID.preview.w, GRID.preview.h, undefined, "FAST");
      }
    }
  }

  /* ====== ПАРАМЕТРЫ ЗАКАЗА ====== */
  const rightX = GRID.rightColX + 5;
  let ry = GRID.paramsY;
  setBold(); doc.setFontSize(14); doc.setTextColor(0);
  ...
  /* ====== КЛИШЕ ====== */
  // --- Восстанавливаем размеры логотипов, если клиент не прислал sizeMM ---
  if (args.logos && typeof args.logos === "object") {
    const dims = {
      W: Number(args.widthMM) || 0,
      H: Number(args.heightMM) || 0,
      D: Number(args.depthMM) || 0,
    };

    const getFaceMM = (side: string) => {
    ...
    for (const [side, logos] of Object.entries(args.logos)) {
      const list = Array.isArray(logos) ? logos : [logos];
      const face = getFaceMM(side);

      for (const it of list) {
        // если sizeMM уже есть — не трогаем
        if ((it as any).sizeMM && (it as any).sizeMM.w && (it as any).sizeMM.h) continue;

        // если есть проценты — пересчёт
        if ((it as any).pctW && (it as any).pctH) {
          const kW = (it as any).pctW > 1 ? (it as any).pctW / 100 : (it as any).pctW;
          const kH = (it as any).pctH > 1 ? (it as any).pctH / 100 : (it as any).pctH;
          (it as any).sizeMM = { w: Math.round(face.w * kW * 10) / 10, h: Math.round(face.h * kH * 10) / 10 };
          continue;
        }

        // если есть scale или scaleX/Y — пересчёт
        const s  = Number((it as any).scale) || Number((it as any).sx) || Number((it as any).scaleX) || 0;
        const sy = Number((it as any).sy) || Number((it as any).scaleY) || 0;
        const k  = sy > 0 ? Math.min(s, sy) : s;
        if (k > 0) {
          const kk = k > 2 ? k / 100 : k; // если scale = 25, значит 25%
          (it as any).sizeMM = { w: Math.round(face.w * kk * 10) / 10, h: Math.round(face.h * kk * 10) / 10 };
          continue;
        }

        // если совсем ничего нет — дефолт 30×30 мм
        (it as any).sizeMM = { w: 30, h: 30 };
      }
    }
  }
  ...
  /* ====== СОГЛАСОВАНИЕ ====== */
  {
    const y = ((doc as any).lastAutoTable?.finalY ?? (GRID.colors.y + 22)) + 70;
    setBold(); doc.setFontSize(11); doc.setTextColor(0);
    doc.text("Согласование", M, y);

    const boxTop = y + 1.5;
    const boxH = 24;
    // если нужна рамка — можно вернуть строки ниже
  ...
  /* ===== ДОБАВКА НИЗА МЕНЕДЖЕРСКОГО ТЗ (КЛИШЕ/ПОЛОСКУ НЕ ТРОГАЕМ) ===== */
  try {
    // 1) Пересчёт — как на панели
    const inputs = {
      widthMM: Number(args.widthMM) || 0,
      heightMM: Number(args.heightMM) || 0,
      depthMM: Number(args.depthMM) || 0,
      quantity: Math.max(1, Math.floor(Number(args.quantity) || 1)),
      currency: (args as any)?.pricing?.currency || "₽",
      boxType: args.boxType,
      baseBoard: args.baseBoard,
      wrapPaper: args.wrapPaper,
      print: args.print,
      finishing: args.finishing,
      ...(args.boxType === "casket"
        ? { wrapLamination: (args as any).wrapLamination ?? "none" }
        : { lamination: (args as any).lamination ?? "none" }),
      innerPaper: (args.boxType === "lidBottom" || args.boxType === "casket")
        ? (args as any).wrapPaper : "none",
      innerMode: (args as any).innerMode ?? "none",
      drawerSleeveInner: (args as any).drawerSleeveInner ?? false,
    };
    const out = estimateCost(inputs);
    const currency = inputs.currency;

    // 2) Безопасный старт НИЖЕ клише/таблиц
    const MARGIN = (typeof PDF_LAYOUT !== "undefined" && PDF_LAYOUT?.margin) ? PDF_LAYOUT.margin : 10;
    const pageH  = doc.internal.pageSize.getHeight();
    let y = ((doc as any).lastAutoTable?.finalY || 0) + 8;
    try { /* @ts-ignore */ if (typeof ry === "number" && isFinite(ry)) y = Math.max(y, ry + 8); } catch {}
    if (!Number.isFinite(y) || y < MARGIN + 10) y = MARGIN + 10;

    // Перенос страницы при риске переполнения
    const estH = 72;
    if (y + estH > pageH - MARGIN - 6) { doc.addPage(); y = MARGIN; }

    // 3) Рисуем недостающие строки (ТОЛЬКО низ)
    doc.setFont("NotoSans", "normal"); doc.setFontSize(10); doc.setTextColor(0);

    const workPlusFin = Number(out.workCost || 0) + Number(out.finishingCost || 0);
    doc.text(`Работы + Отделка: ${fmt(workPlusFin, currency)}`, MARGIN, y); y += 5;

    doc.text(`Накладные: ${fmt(out.overhead, currency)}`, MARGIN, y); y += 7;

    // Расход листов
    doc.setFont("NotoSans", "bold"); doc.setFontSize(11);
    doc.text("Расход листов (1000×700 мм)", MARGIN, y); y += 5;
    doc.setFont("NotoSans", "normal"); doc.setFontSize(11);

    const board = (out as any)?.sheets?.board ?? 0;
    const wrap  = (out as any)?.sheets?.wrap  ?? 0;
    const inner = (out as any)?.sheets?.inner ?? 0;

    doc.text(`Картон: ${String(board)} лист.`, MARGIN, y); y += 5;
    doc.text(`Оклейка: ${String(wrap)} лист.`, MARGIN, y); y += 5;
    doc.text(`Кашировка: ${String(inner)} лист.`, MARGIN, y); y += 7;

    // Итог
    doc.setFont("NotoSans", "bold"); doc.setFontSize(11);
    const total   = fmt(out.total,   currency);
    const perUnit = fmt(out.perUnit, currency);
    doc.text(`Итого: ${total}      (≈ ${perUnit} за шт.)`, MARGIN, y);
    doc.setFont("NotoSans", "normal");

    try { /* @ts-ignore */ ry = y + 4; } catch {}
  } catch {
    // не роняем генерацию
  }
  /* ===== КОНЕЦ ДОБАВКИ НИЗА ===== */
